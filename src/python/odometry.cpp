...
void define_odometry(py::module_& m) {
  ...
  auto est_frame =
    py::class_<glim::EstimationFrame, std::shared_ptr<glim::EstimationFrame>>(m, "EstimationFrame", "Odometry estimation frame")
      ...
      .def_property_readonly(
        "v_world_imu",
        [](const glim::EstimationFrame& f) -> Eigen::Vector3d { return f.v_world_imu; },
        "IMU velocity in the world frame")
      .def_property_readonly(
        "imu_bias",
        [](const glim::EstimationFrame& f) -> Eigen::Matrix<double, 6, 1> { return f.imu_bias; },
        "IMU bias [acc, gyro]")
      .def_readonly("cov_computed", &glim::EstimationFrame::cov_computed, "Whether the covariances have been computed")
      .def_property_readonly(
        "pose_cov",
        [](const glim::EstimationFrame& f) -> Eigen::Matrix<double, 6, 6> { return f.pose_cov; },
        "Pose covariance")
      .def_property_readonly(
        "vel_cov",
        [](const glim::EstimationFrame& f) -> Eigen::Matrix3d { return f.vel_cov; },
        "Velocity covariance")
      .def_property_readonly(
        "bias_cov",
        [](const glim::EstimationFrame& f) -> Eigen::Matrix<double, 6, 6> { return f.bias_cov; },
        "IMU bias covariance")
      ...
      .def_property_readonly("raw_frame", [](const glim::EstimationFrame& f) { return unconst(f.raw_frame); }, "Raw input point cloud (LiDAR frame)");
  ...
  py::class_<glim::OdometryEstimationBase, std::shared_ptr<glim::OdometryEstimationBase>>(m, "OdometryEstimationBase", "Odometry estimation base class")
    .def_static(
      "load_module",
      &glim::OdometryEstimationBase::load_module,
      py::arg("so_name"),
      "Load an odometry estimation module from a shared library (e.g., libodometry_estimation_cpu.so)")
  ...
  py::class_<glim::OdometryEstimationCPU, glim::OdometryEstimationBase, std::shared_ptr<glim::OdometryEstimationCPU>>(
    m,
    "OdometryEstimationCPU",
  ...
  py::class_<glim::OdometryEstimationCT, glim::OdometryEstimationBase, std::shared_ptr<glim::OdometryEstimationCT>>(
    m,
    "OdometryEstimationCT",
    "LiDAR-only odometry estimation based on CT-GICP")
  ...
  py::class_<glim::OdometryEstimationGPU, glim::OdometryEstimationBase, std::shared_ptr<glim::OdometryEstimationGPU>>(
    m,
  ...
  py::class_<glim::AsyncOdometryEstimation, std::shared_ptr<glim::AsyncOdometryEstimation>>(
    m,
    "AsyncOdometryEstimation",
    "Wrapper to run odometry estimation in a background thread")
    .def(
